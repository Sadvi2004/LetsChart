...
export const useChatStore = create((set, get) => ({
    ...
    messages: [], //messages of current conversations
    loading: false,
    error: null,
    onlineUsers: new Map(), //map of online users
    typingUsers: new Map(), //map of typing users
    ...
    initsocketListeners: () => {
        ...
        //listener for user online/offline status
        ...
        //emit status check for al users in conversation list
    ...
    sendMessage: async (formData) => {
        ...
        if (conversations?.data?.length > 0) {
            const conversation = conversations.data.find((conv) => conv.participants.some((p) => p._id === senderId) && conv.participants.some((p) => p._id === receiverId));
            if (conversation) {
    ...
    markMessagesAsRead: async (conversationId) => {
        ...
        try {
            ...
            console.error("Fail to mark messages as read:", error);
        }
    },
